...
replace month = 12 if month ==1
replace month = month - 1 if month !=1
	...
	foreach sta of local st_list{
		foreach term of local terms {
			...
			twoway ///
				...
				(scatter unemployment_rate `term' if stname == "`sta'", msymbol(o) mcolor(grey)) ///
				(line `term'_p_`sta' `term' if stname == "`sta'", title("`sta' Monthly Google Search for '`term'' vs. EPOP Rate", size(medium)) xtitle("Google Searches for "`term'"") ytitle("EPOP") caption("R2 Linear = `r2'") graphregion(color(white)) plotregion(color(white)) legend(label(1 "Nonparametric CI") label(2 "Nonparametric") label(3 "") label(4 "Linear")) lcolor(blue))
...
putpdf begin
	...
	putpdf save "$images/MonthLaggedControls.pdf", replace
